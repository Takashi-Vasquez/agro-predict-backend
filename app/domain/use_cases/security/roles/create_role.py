...
class CreateRoleUseCase:
    ...
    def execute(self, name: str, description: str | None = None, status: str = "ACTIVE") -> Role:
        existing = self.role_repo.get_by_name(name)
        if existing:
            raise ValueError("Ya existe un rol con ese nombre")
        return self.role_repo.create(name=name, description=description, status=status)
